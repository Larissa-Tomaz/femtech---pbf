...
public class HyperledgerService {
    ...
    public models.User registerUser(String profileName, String registrationDate, String paymentPlan, Boolean subscriptionStatus, 
                         Auth.ROLE role, String username, String password,
                         String age, String menstrualCycleLength, String periodLength, String mainContraceptiveMethod) throws IOException, ContractException, InterruptedException, TimeoutException {
        
        Gateway gateway = this.getGateway();
        Contract contract = this.getContract(gateway);

        //System.out.println("vou enviar o create user para o hyperledger");

        models.User user = new models.User(
            new UserAccountData(profileName, registrationDate, paymentPlan, subscriptionStatus),
            new UserProfileData(age, menstrualCycleLength, periodLength, mainContraceptiveMethod),
            role,
            username,
            password
        );
        ...
        //String serializedConsent = this.genson.serialize(user);
        //System.out.println("Serialized User: " + serializedConsent);
        contract.submitTransaction("createUser",
                            username, 
    ...
    public models.User login(String username, String hashedPassword) throws IOException, ContractException {
        ...
        //System.out.println("vou chamar o contrato para logar o user");

        byte[] result = contract.evaluateTransaction("login", username, hashedPassword);
        //System.out.println("eu recebi esse user do hyperledger:" + new String(result));
        if (result == null || result.length == 0) {
            System.err.println("User not registered");
            return null;
        }
        gateway.close();
        return this.genson.deserialize(new String(result), models.User.class);
    }
    ...
    public boolean deleteUser(String username) throws IOException, ContractException,
     ...
     public UserAccountData getAccountData(String username) throws IOException, ContractException,
            InterruptedException, TimeoutException {
        Gateway gateway = this.getGateway();
        Contract contract = this.getContract(gateway);

        byte[] result = contract.submitTransaction("getAccountData", username);

        gateway.close();
        return this.genson.deserialize(new String(result), UserAccountData.class);

    }

    public UserProfileData getProfileData(String username) throws IOException, ContractException,
            InterruptedException, TimeoutException {
        Gateway gateway = this.getGateway();
        Contract contract = this.getContract(gateway);

        byte[] result = contract.submitTransaction("getProfileData", username);

        gateway.close();
        return this.genson.deserialize(new String(result), UserProfileData.class);
